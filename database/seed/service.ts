...
const CLUSTER_1_URI = `${process.env.DB_URI_CLUSTER_1_READONLY}`;
...
type Cluster = "cluster1" | "cluster2" | "local";
...
export class SeedService {
  ...
  async connect() {
    this.connections.cluster1 = await connect(CLUSTER_1_URI);
  ...
  private getRemoteConnection(database: Database) {
    switch (database) {
      ...
      case Database.Social:
        return this.connections.cluster2;
      case Database.Governance:
      case Database.Eth:
        return this.connections.cluster1;
      default:
