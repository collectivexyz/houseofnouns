...
export function ProposalsList(props: Props) {
  ...
  const fuse = new Fuse(proposals, {
    ...
    keys: [
      { name: "title", weight: 3 },
      { name: "profile.username", weight: 1.8 },
      "description",
    ],
    distance: 10000,
  });
  ...
  return (
    <div className="flex h-full flex-col lg:h-[calc(100%-32px)]">
      ...
      <ScrollShadow>
        {(nameIncludes
          ? fuse.search(nameIncludes).map((result) => result.item)
          : proposals
        )
          .sort((a, b) => {
            if (nameIncludes) {
            ...
            return (
              (mapFunc(get(b, sortValue)) - mapFunc(get(a, sortValue))) *
              sortDirection *
              -1
            );
          })
