import { importSPKI } from "jose";

//Defaults
const SANDBOX_ID = "9dce297e-b3c9-44f4-8006-8d5927c59954";
const DYNAMIC_SANDBOX_KEY =
  "-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----";
const DYNAMIC_LIVE_KEY =
  "-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----";

const DEFAULT_ENVIRONMENT_ID =
  process.env.NEXT_PUBLIC_DYNAMIC_ENVIRONMENT_ID || SANDBOX_ID;

export const getDynamicEnvironmentId = () => {
  return DEFAULT_ENVIRONMENT_ID;
};

export const DYNAMIC_AUTH_CONFIG = () => {
  const environmentId = getDynamicEnvironmentId();
  const appName = "House of Nouns";

  return {
    appLogoUrl:
      "https://pbs.twimg.com/profile_images/1544778556714225664/0DabCavf_400x400.jpg",
    appName,
    environmentId,
    privacyPolicyUrl: "https://collective.xyz/info/privacy",
    termsOfServiceUrl: "https://collective.xyz/tos.pdf",
    multiWallet: true,
  };
};

export const getEnvType = (): "sandbox" | "live" => {
  return DEFAULT_ENVIRONMENT_ID === SANDBOX_ID ? "sandbox" : "live";
};

export const getPublicKey = async () => {
  const spki = getEnvType() === "live" ? DYNAMIC_LIVE_KEY : DYNAMIC_SANDBOX_KEY;

  return await importSPKI(spki, "RS256");
};
